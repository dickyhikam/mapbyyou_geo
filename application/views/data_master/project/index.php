                ...
                <div class="modal-body">
                    <form id="form_add">
                        ...
                        <fieldset class="form-group">
                            ...
                            <select class="form-control" id="type" name="type" data-placeholder="Choose ...">
                                <option value="">Choose Type Project</option>
                        ...
                        <fieldset class="form-group">
                            ...
                            <input class="form-control" id="order" name="order" placeholder="Enter project order" onkeypress="return hanyaAngka(event, true);">
                            <div class="invalid-feedback">
                                Order cannot be empty.
                            </div>
                        </fieldset>
                        <fieldset class="form-group">
                            <label for="limit">Limit Claim*</label>
                            <input class="form-control" id="limit" name="limit" placeholder="Enter limit claim" onkeypress="return hanyaAngka(event, true);">
                            <div class="invalid-feedback">
                                Limit claim cannot be empty.
                            </div>
                        </fieldset>
                        <fieldset class="form-group">
...
<script>
    ...
    $(document).ready(function() {
        ...
        var_table = $('#table').DataTable({
            ajax: "<?php echo base_url(); ?>DM_Project/table",
            columnDefs: [{
        ...
        $('#limit').keyup(function() {
            var txt = $(this).val();
            if (txt !== '') {
                $('#limit').removeClass('is-invalid'); //remove the written class name
            } else {
                $('#limit').addClass('is-invalid'); //add the written class name
            }
        });
    ...
    function save() {
        ...
        if (name !== '' && order !== '' && limit !== '' && type !== '') {
            ...
            var url;
            if (condition === 'add') {
                url = "<?php echo base_url(); ?>DM_Project/add";
            } else {
                url = "<?php echo base_url(); ?>DM_Project/update";
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status === "success") {
                        toastr["success"]("Data project success to " + condition + ".", "Success");

                        reload();
                        $('#modal_add').modal('hide'); // show bootstrap modal
                    } else {
                        toastr["error"]("Data project failed to " + condition + ", try again!", "Failed");
                    }
                },
    ...
    function modal_edit(id) {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>DM_Project/rowdata/" + id, //link access data
    ...
    function confirm() {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>DM_Project/delete", //link access data
            type: "POST", //action in form
            dataType: "JSON", //accepted data types
            ...
            success: function(data) {
                ...
                //show notification
                if (data.status === "success") {
                    toastr["success"]("Data project deleted successfully.", "Berhasil");

                    $('#form_confirm')[0].reset(); // reset form on modals
                    $('#modal_confirm').modal('hide'); // show bootstrap modal
                    reload();
                } else {
                    toastr["error"]("Data project failed to delete, try again!", "Gagal");
                }
            },
            error: function(jqXHR, textStatus, errorThrown) {
                //return button function
