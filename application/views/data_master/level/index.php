...
<script>
    ...
    $(document).ready(function() {
        ...
        var_table = $('#table').DataTable({
            ajax: "<?php echo base_url(); ?>DM_Level/table",
            columnDefs: [{
    ...
    function save() {
        ...
        if (name !== '') {
            ...
            var url;
            if (condition === 'add') {
                url = "<?php echo base_url(); ?>DM_Level/add";
            } else {
                url = "<?php echo base_url(); ?>DM_Level/update";
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status === "success") {
                        toastr["success"]("Data level success to " + condition + ".", "Success");

                        reload();
                        $('#modal_add').modal('hide'); // show bootstrap modal
                    } else {
                        toastr["error"]("Data level failed to " + condition + ", try again!", "Failed");
                    }
                },
    ...
    function modal_edit(id) {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>DM_Level/rowdata/" + id, //link access data
    ...
    function confirm() {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>DM_Level/delete", //link access data
            type: "POST", //action in form
            dataType: "JSON", //accepted data types
            ...
            success: function(data) {
                ...
                //show notification
                if (data.status === "success") {
                    toastr["success"]("Data level deleted successfully.", "Success");

                    $('#form_confirm')[0].reset(); // reset form on modals
                    $('#modal_confirm').modal('hide'); // show bootstrap modal
                    reload();
                } else {
                    toastr["error"]("Data level failed to delete, try again!", "Failed");
                }
            },
            error: function(jqXHR, textStatus, errorThrown) {
                //return button function
